...
export default function App() {
  ...
  const features = [
    { icon: <BarChart3 className="w-5 h-5" />, title: "Financial Analysis", text: "Ratios, dashboards, and insights that drive decisions." },
    { icon: <Database className="w-5 h-5" />, title: "Digital Transformation", text: "Excel automation, Power BI, QuickBooks & Notion workflows." },
    { icon: <Code className="w-5 h-5" />, title: "Creative Builds", text: "Web design, branding identity, and marketing content." },
  ];
  const projects = [
    { title: "Amman Stock Exchange – Petrochemical Analysis", tag: "Excel • Ratios • Presentation", desc: "End-to-end financial analysis with ratio evaluation and executive summary." },
    { title: "RFID Inventory for Agriculture", tag: "Data • Power BI • RFID", desc: "Tracking palms & plants with RFID tags, analytics-ready structure." },
    { title: "Four Seasons Contracting Project", tag: "Accounting • Cost Control", desc: "Financial & administrative accounting on a large construction project." },
  ];
  const services = [
    { title: "Accounting & Reporting", points: ["Bookkeeping & GL", "Management reports", "Cash control"] },
    { title: "Dashboards & BI", points: ["Power BI visuals", "Excel automation", "KPI tracking"] },
    { title: "Branding & Web", points: ["Wix & landing pages", "Logo & identity", "Content & campaigns"] },
  ];

  function handleSubmit(e) {
    e.preventDefault();
    alert(`Thanks! I will reach out to ${email}.`);
    setEmail(""); setMsg("");
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 text-slate-900">
        ...
        <div className="max-w-6xl mx-auto px-4 py-16 md:py-24 grid md:grid-cols-2 gap-8 items-center">
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
            ...
            <p className="mt-4 text-slate-600 md:text-lg">
              Accountant & Financial Analyst with a passion for digital transformation. I build smart Excel/Power BI systems and present insights with clean, modern design.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <a href="#projects"><button className="rounded-2xl px-4 py-2 bg-slate-900 text-white">View Projects</button></a>
              <a href="#contact"><button className="rounded-2xl px-4 py-2 border border-slate-300">Contact Me <ArrowRight className="inline-block ml-2 w-4 h-4" /></button></a>
            </div>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.6, delay: 0.1 }}>
            <div className="relative rounded-3xl shadow-xl p-1 bg-gradient-to-br from-slate-100 to-slate-200">
              <div className="rounded-3xl bg-white p-6 h-full">
                <div className="aspect-video rounded-2xl bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-slate-100 via-slate-50 to-white border border-slate-200 flex items-center justify-center">
                  <div className="text-center p-6">
                    <p className="text-sm tracking-widest text-slate-500">Preview</p>
                    <h3 className="text-xl font-semibold mt-2">Clean Visual Reporting</h3>
                    <p className="text-slate-600 mt-1">Minimal, executive-ready charts & scorecards.</p>
                  </div>
                </div>
        ...
        <div className="max-w-6xl mx-auto px-4 grid md:grid-cols-2 gap-8 items-start">
          <div>
            ...
            <p className="mt-3 text-slate-600 leading-relaxed">
              I’m an accountant based in Saudi Arabia with 3+ years of practical experience across contracting and retail.
              I enjoy building data models, automating reports, and turning numbers into clear actions.
              Outside finance, I craft simple brands and landing pages that help businesses look sharp.
            </p>
            <div className="mt-5 flex flex-wrap gap-2 text-sm">
              <Badge>Accounting</Badge><Badge>Financial Analysis</Badge><Badge>Power BI</Badge>
              <Badge>Excel</Badge><Badge>QuickBooks</Badge><Badge>Wix / Web</Badge>
            </div>
          </div>
          <div className="rounded-3xl border p-6">
            <div className="grid sm:grid-cols-3 gap-4">
              <Stat kpi="3+" label="Years Experience" />
              <Stat kpi="8+" label="Major Projects" />
              <Stat kpi="FMVA" label="Certification" />
            </div>
            <ul className="mt-5 space-y-2 text-slate-700 text-sm">
              <li className="flex gap-2 items-start"><ArrowRight className="w-4 h-4 mt-1" /> Real projects: ASE analysis, RFID inventory, Four Seasons contracting.</li>
              <li className="flex gap-2 items-start"><ArrowRight className="w-4 h-4 mt-1" /> Tooling: Excel automation, Power BI, QuickBooks, Notion.</li>
              <li className="flex gap-2 items-start"><ArrowRight className="w-4 h-4 mt-1" /> Edge: blend of finance rigor + creative presentation.</li>
            </ul>
          </div>
        </div>
      </section>
      ...
      <section id="contact" className="py-14 md:py-20 bg-white">
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid md:grid-cols-2 gap-8 items-start">
            ...
            <div className="rounded-3xl border p-6">
              <form onSubmit={(e)=>{ e.preventDefault(); alert(`Thanks! I will reach out to ${email}.`); setEmail(''); setMsg(''); }} className="space-y-3">
      ...
      <footer className="py-10 text-center text-sm text-slate-500">
        © {new Date().getFullYear()} Osama Marashdeh · Finance & Creative · Built with ❤️
      </footer>
    </div>
  );
}
